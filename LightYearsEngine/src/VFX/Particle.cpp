...
namespace ly
{
	...
	{
		mVelocity = RandomUnitVactor() * RandomRange(minSpeed, maxSpeed);
	}
	...
	void Particle::Fade(float deltaTime)
	{
		...
		GetSprite().setColor(LerpColor(GetSprite().getColor(),
			sf::Color(255, 255, 255, 0), elapsedTime / mLifeTime));

		GetSprite().setScale(LerpVector(GetSprite().getScale(),
			sf::Vector2f{ 0,0 }, elapsedTime / mLifeTime));
	}
