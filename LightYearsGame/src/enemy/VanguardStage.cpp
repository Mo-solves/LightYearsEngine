...
namespace ly
{
	...
	void VanguardStage::StartStage()
	{
		...
		mRightSpawnLoc = sf::Vector2f(windowsSize.x - mSpawnDistanceToEdge, -100.f);
	...
	void VanguardStage::SpawnVanguard()
	{
		...
		{
			TimerManager::Get().ClearTimer(mSpawnTimerHandle);
			mSwitchTimerHandle = TimerManager::Get().SetTimer(GetWeakRef(),
				&VanguardStage::SwitchRow, mSwitchInterval, false);
			mCurrentRowVanguardCount = 0;
		}
	}
	...
	void VanguardStage::SwitchRow()
	{
		...
		mSpawnTimerHandle = TimerManager::Get().SetTimer(GetWeakRef(), &VanguardStage::SpawnVanguard,
			mSpawnInterval, true);
