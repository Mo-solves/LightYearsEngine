...
namespace ly
{
	...
	{
		mSpawnTimerHandler = TimerManager::Get().SetTimer(GetWeakRef(),
			&TwinBladeStage::SpawnTwinBlade, mSpawnInterval, true);
	}
	...
	{
		TimerManager::Get().ClearTimer(mSpawnTimerHandler);
	}
}